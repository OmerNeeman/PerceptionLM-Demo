...
| S4 | **sent back** | per-scale ranking, latency (9-18 ms vs 200 ms) and the uncentered-PCA diagnosis all good. Three items back: tests were truncating the real tileplan; the weak-match boolean was overfit; `EXPORT_DIM` raised 128 -> 384. **U-3 amended** (no boolean achievable) and **F-2a answered**: PCA-128 changed ~29% of top-10 | `briefs/S4_fix.md` dispatched |
...
| 6 | `accelerate` 1.14.0 installed but unused — no candidate needed sharded loading | Installed speculatively during S0 before that was known | Negligible; one unused package in the env. Recorded so a future reproducibility audit does not treat it as load-bearing | PM, low-cost |
